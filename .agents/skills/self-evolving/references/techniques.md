...
- worktree 环境变量自动接入(git 2.50 实测):仓库内 githooks/post-checkout + git config core.hooksPath githooks(相对路径按各 worktree 根解析,随检出分发);git worktree add 会触发 post-checkout(prev=全 0 SHA、flag=1),hook cwd 即新 worktree 根;git 忽略 post-checkout 退出码,hook 各路径显式 exit 0 即永不阻断检出。